...
from app.schemas import UserCreate, UserLogin, UserOut, Token, ResponseModel
...
async def register_user(user_data: UserCreate, db: AsyncSession = Depends(get_db)):
    existing = await get_user_by_email(db=db, email=user_data.email)
    if existing:
        return error_response(
            status_code=status.HTTP_400_BAD_REQUEST, message="Email already taken"
        )
        
    existing = await get_user_by_username(db=db, username=user_data.username)
    if existing:
        return error_response(
