...
@router.post("/", response_model=ResponseModel[Reader])
...
@router.get("/", response_model=ResponseModel[List[Reader]])
...
@router.get("/{reader_id}", response_model=ResponseModel[Reader])
...
@router.put("/{reader_id}", response_model=ResponseModel[Reader])
async def update_reader(
    ...
    return success_response(message="Updated reader successfully", data=updated_reader)

@router.delete("/{reader_id}",  response_model=ResponseModel[None])
async def delete_reader(reader_id: str, db: AsyncSession = Depends(get_db)):
    ...
    return success_response(message="Deleted reader successfully")